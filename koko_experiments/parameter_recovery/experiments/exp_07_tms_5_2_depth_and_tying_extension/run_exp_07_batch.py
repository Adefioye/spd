...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run exp_07 TMS depth/tied-vs-untied target training, SPD, and analysis on CPU."
    )
    parser.add_argument(
        "config_path",
        type=Path,
...
def _materialize_target_config(
    target_config: TMSTrainConfig, wandb_project: str
) -> TMSTrainConfig:
...
def _run_spd_for_target(target_checkpoint_path: Path, spd_config: Config) -> Path:
    target_run_info = TMSTargetRunInfo.from_path(target_checkpoint_path)
    target_model = TMSModel.from_run_info(target_run_info).to("cpu")
    target_model.eval()
...
def main() -> None:
    ...
    for run_spec in batch_config.runs:
        ...
        if args.reuse_existing_targets:
            ...
            target_run_dir = run_train(target_config, device="cpu")
